...
from_addr = 'khewalkarpiyush2005@gmail.com'

data = pd.read_csv('Automated Mail Merge/input/team_directory.csv')

subject = open(file='Automated Mail Merge/input/subject.txt', mode='r').read()
...
data.to_csv('Automated Mail Merge/input/team_directory.csv')
print(set_of_emails)
list_of_emails = list(set_of_emails)

#attaches the from_addr, to_addr, subject, and the body of the mail
text_content = open(file='Automated Mail Merge/input/team_directory.csv', mode='r').read()
for email in list_of_emails:
    ...
    if os.listdir('Automated Mail Merge/input/attachments') is not []:
        #attaching multiple attachments
        filenames = os.listdir('Automated Mail Merge/input/attachments')
        file_directories = []
        for file_dir in filenames:
            file_directories.append(os.path.join('Automated Mail Merge/input/attachments', file_dir))
